...
# include "transparency2/window_frames_renderable2.h"
# include "transparency2/transparent_windows_renderable2.h"
// utilities
...
namespace application {
  ...
  private:
    void do_add( graphics_origin::application::renderable* r ) override
    {
      ...
      transparent_windows_renderable2* windows = dynamic_cast< transparent_windows_renderable2* >( r );
      if( windows )
        {
          if( m_windows )
            {
    ...
    graphics_origin::application::transparent_windows_renderable2* m_windows;
  };

  class simple_gl_window2
    : public qt::window {
  public:
    simple_gl_window2( QQuickItem* parent = nullptr )
      : qt::window( parent )
    {
      initialize_renderer( new transparency_gl_renderer2 );
      ...
      auto windows = new transparent_windows_renderable2( transparent_program, angle_divisions );
      auto frames = new window_frames_renderable2( frame_program, angle_divisions );
...
# include "transparency2/rotating_camera2.h"
# include "transparency2/simple_qml_application2.h"
...
int main( int argc, char* argv[] )
{
  ...
  qmlRegisterType<graphics_origin::application::simple_gl_window2>( "GraphicsOrigin", 1, 0, "GLWindow" );
  qmlRegisterType<graphics_origin::application::rotating_camera2   >( "GraphicsOrigin", 1, 0, "GLCamera" );

  // Load the main QML describing the main window into the simple QML application.
  std::string input_qml = graphics_origin::tools::get_path_manager().get_resource_directory( "qml" ) + "4_transparency2.qml";
  graphics_origin::application::simple_qml_application2 app;
