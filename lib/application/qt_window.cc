...
namespace graphics_origin {
  namespace application {
    ...
    window::window( QQuickItem* parent ) :
        QQuickItem( parent ), threaded_renderer( nullptr ), render_thread( new QThread() )
    {
      ...
      // add this instance to the list of existing instances.
      window_instances << this;

      connect( render_thread, SIGNAL(finished()), render_thread, SLOT(deleteLater()));
    }
    ...
    qreal window::get_ms_per_frame()
    {
      return threaded_renderer->period.get_period() * 1000.0;
    }
    ...
    {
      if( threaded_renderer )
        threaded_renderer->set_surface_dimensions(
